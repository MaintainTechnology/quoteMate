...
_Converted from `stage1-05-sop.html`._
...
**This SOP describes the voice-first pipeline.** Per [docs/strategy.md](strategy.md) and [CLAUDE.md](../CLAUDE.md), the QuoteMate **v1 wedge is portal-first** (typed intake) — voice is deferred to the v3+ premium tier on COGS grounds. **Stages 04 and 05 are stack-identical for portal and voice** — only Stage 03 changes. If you're building v1, do Foundation 1 + 2 + Stages 04 and 05, replace Stage 02 (Twilio) and Stage 03 (Vapi) with a typed form that POSTs to `/api/intake/structure`, and skip [the no-Twilio path](architecture.html#why-no-n8n) in the architecture doc.

## Before you start, read this once.

You're going to build a phone-answering AI. By the end, a homeowner can dial a number, have a conversation with an AI receptionist, and a draft quote will appear in your database 45 seconds after they hang up. The build is split into **the first 5 of 10 stages** from the architecture diagram, plus two **foundation blocks** in between that wire the plumbing.

**What's deliberately NOT in this SOP:** stages 06–10 of the wireframe — confidence routing, customer Good/Better/Best portal, availability nudge, follow-up engine, job-won. See [wireframe.html](wireframe.html) for those. Of the four QuoteMate agents (_Quote Drafter_, _Quote Reviewer_, _Inspection Coordinator_, _Conversion Engine_), only **Quote Drafter** is built here — the other three live in the v2/v3 stages this guide skips.

This guide pairs with three reference docs: [architecture.html](architecture.html) shows the system shape; [build-guide.html](build-guide.html) has the full code blocks; [wireframe.html](wireframe.html) shows the full 10-stage product flow and the 4-agent decomposition. **This file is the click-by-click order.** Open it in one browser tab and keep building in the other.
...
3.  Heads-up: this file genuinely deviates from the build-guideThe build-guide originally wrote `anthropic.embedding('voyage-3')`, but the `@ai-sdk/anthropic` provider has no `.embedding()` method (Anthropic doesn't sell embeddings — Voyage AI is a separate vendor they recommend). The current build-guide and the deployed `lib/intake/embed.ts` instead call Voyage's REST API directly and fall back to a deterministic 1536-dim stub when `VOYAGE_API_KEY` isn't set, so the pipeline still completes end-to-end during local testing. Read the comment block at the top of `lib/intake/embed.ts` for the full deviation rationale.
4.  Voyage vs OpenAIIf you'd prefer to use OpenAI for embeddings instead of Voyage, swap the implementation per the note at the bottom of step 7 — OpenAI's `text-embedding-3-small` outputs 1536 dims natively, so no padding is needed.
...
3.  Paste the four toolsFrom [build-guide.html step 8](build-guide.html#step-8), copy the block that defines `lookupAssembly`, `lookupMaterial`, `applyMarkup`, `flagInspectionNeeded`. Each tool uses `inputSchema:` (AI SDK v5 rename of the older `parameters:`) — if you find a copy of the snippet that says `parameters: z.object(...)` it's stale and won't compile.
4.  Save
...
2.  Paste the runEstimation functionFrom [build-guide.html step 8](build-guide.html#step-8), copy the runner block. The key part is `generateText({ ..., tools, stopWhen: stepCountIs(10) })` — that's what gives Claude the agency to call tools in a loop. **Two AI SDK v5+ details to be aware of:** (1) the import is `import { generateText, stepCountIs } from 'ai'` — older docs may show `maxSteps: 10`, which was renamed in v5 (the default is `stepCountIs(1)`, so without this Opus would never enter the tool loop); (2) the deployed runner contains a `parseJsonFromText` helper that finds the first balanced `{ ... }` block in Opus's response — Opus often prefixes its JSON with reasoning prose like _"Calculation: …"_, which a naive `JSON.parse(result.text)` would choke on.
3.  Save
...
-   `stopWhen: stepCountIs(10)` exhausted — Opus tried more than 10 tool calls. Either increase the limit or simplify the prompt.
