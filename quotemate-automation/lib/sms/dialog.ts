...
//
/** Only exact server-approved URLs may leave the dialog. The legacy sourceText
 * argument is retained for callers, but transcript text is not quote authority.
 * Token lookup and ownership checks belong to the deterministic quote action.
 */
export function repairQuoteLinks(reply: string, _sourceText: string, verifiedUrls: readonly string[] = []): string {
  return guardGeneratedQuoteLinks(reply, verifiedUrls)
}
...
export async function decideNextTurn(args: {
  ...
  /** Resolved by a tenant/customer-owned database lookup, never by history. */
  verifiedQuoteUrls?: readonly string[]
  ...
  const repaired = repairQuoteLinks(cleaned, linkSource, args.verifiedQuoteUrls)
  if (repaired !== cleaned) {
  ...
  return {
    ...object,
    reply_to_send: object.action === 'finish'
      ? 'Thanks, I have the details for this request. A saved draft still needs the tradie’s review before a quote can be shared.'
      : repaired,
  }
}
